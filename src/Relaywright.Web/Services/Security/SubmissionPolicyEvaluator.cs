...
public sealed class SubmissionPolicyEvaluator
{
        ...
        long declaredSizeBytes)
    {
        ...
        var effectiveSizeLimit = Min(profile.MaxMessageSizeBytes, globalSizeLimit);
        if (effectiveSizeLimit is not null && declaredSizeBytes > effectiveSizeLimit.Value)
        {
            return SubmissionPolicyDecision.Deny(
                $"Message size exceeds the allowed limit of {effectiveSizeLimit.Value} bytes.");
        }
        ...
        int recipientNumber)
    {
        ...
        var effectiveRecipientLimit = Min(profile.MaxRecipientsPerMessage, globalRecipientLimit);
        if (effectiveRecipientLimit is not null && recipientNumber > effectiveRecipientLimit.Value)
        {
            return SubmissionPolicyDecision.Deny(
                $"Message exceeds the allowed recipient limit of {effectiveRecipientLimit.Value}.");
        }
